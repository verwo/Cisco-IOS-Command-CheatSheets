...
``R1(config-router)#default-information originate``| propagate the default static route in OSPF updates
``R1(config-router)#auto-cost reference-bandwidth [nr]``| adjust the reference bandwidth 
...
``R1(config-if)#ip ospf cost [nr]``| manually set the OSPF cost value
